...
export class BaseComponent extends HTMLElement {
    ...
    if (this.allowCustomTemplate) {
      ...
      if (customTplSelector) {
        ...
        if (customTpl) {
          ...
          console.warn(`jam-x template "${customTplSelector}" is not found...`);
        }
      }
    }
  ...
  get localCtx() {
    if (!this.__localCtx) {
      /** @private */
      this.__localCtx = PubSub.registerCtx({}, this);
    }
  ...
  static __parseProp(prop, fnCtx) {
    ...
    /** @type {String} */
    let name;
    if (prop.startsWith(DICT.EXT_CTX_PRFX)) {
      ctx = fnCtx.nodeCtx;
      name = prop.replace(DICT.EXT_CTX_PRFX, '');
  ...
  __initDataCtx() {
    ...
    for (let prop in this.init$) {
      if (prop.startsWith(DICT.EXT_CTX_PRFX)) {
        this.nodeCtx.add(prop.replace(DICT.EXT_CTX_PRFX, ''), this.init$[prop], this.__ctxOwner);
      } else if (prop.includes(DICT.NAMED_CTX_SPLTR)) {
